...
def calcul_age():
    ...
    if int(year1_entry.get()) <= 1900:
        age_show.config(text="")
        messagebox.showerror("ERREUR ANNE ACTUELLE", "Erreur, l'année actuelle que vous entrez doit être comprise entre 1900 et l'infini.")
        checka1 = 0
    # si l'année est inférireure à 1900 alors on affiche une erreur
   ...
   # récupérer l'année 2
    ...
    if total_hours2 > total_hours1:
        age_show.config(text="")
        # on affiche l'erreur
        messagebox.showerror("ERREUR","Erreur, l'année de la date de naissance est plus grande que celle de l'année actuelle, merci de rectifier celle-ci.")
        checka2 = 0
